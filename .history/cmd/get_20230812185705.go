...
Display specific Attribute values of a Server Type  | working
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// For itldims get [IP/TYPE/ATTRIBUTE] [IP/TYPE/ATTRIBUTE]  --- NO COMBINATION B/W KEY & VALUES
		...
		// 		if len(args) > 1 && !strings.Contains(key, args[0]) && !strings.Contains(key, args[1]) {
		// 			continue
		// 		}
		...
		// 	}
		// }
		// For itldims get [IP/TYPE/ATTRIBUTE] [IP/TYPE/ATTRIBUTE]  --- COMBINATION B/W KEY & VALUE
		for key, value := range parseKeyValuePairs(data) {
			if strings.Contains(key, "{") || strings.Contains(key, "}") ||
				strings.Contains(value, "{") || strings.Contains(value, "}") {
				continue
			...
			if !strings.Contains(key, "data") {
				if len(args) > 1 && !strings.Contains(key, args[0]) && !strings.Contains(value, args[1]) {
					continue
				}

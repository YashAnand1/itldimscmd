...
- itldims get <Server IP> <Attribute> | Display specific Attribute values of a Server Type     | Working
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for key, value := range parseKeyValuePairs(data) {
			if len(args) > 1 && strings.Contains(key, "{") || strings.Contains(key, "}") || strings.Contains(key, "data") ||
				strings.Contains(value, "{") || strings.Contains(value, "}") ||
				strings.Contains(key, args[1]) && !strings.Contains(value, args[1]) {
				continue
			}

			if !strings.Contains(key, "data") && (strings.Contains(key, args[0])) && (strings.Contains(value, args[1])) || (strings.Contains(key, args[1])) {

...
s`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			} else if len(args) == 1 && !strings.Contains(args[0], ".") { //if there is only one argument and it DOESNOT conist of "." or serverIP then
				if strings.Contains(key, args[0]) || strings.Contains(value, args[0]) { //also if the first argument contain key or value as first argument

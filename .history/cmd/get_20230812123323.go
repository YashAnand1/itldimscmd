	...
	Run: func(cmd *cobra.Command, args []string) { //This function is to be executed when 'get' subcommand is run
		...
		if len(args) == 1 && (args[0] == "servers") {
			...
			for key := range parseKeyValuePairs(data) {
				...
				// If the IP already exists in the map, append the server type to the existing entry
				if existingType, ok := uniqueIPs[serverIP]; ok {
					uniqueIPs[serverIP] = existingType + "/" + serverType
				} else {
					uniqueIPs[serverIP] = serverType
				}
			}

			for _, serverType := range uniqueIPs {
				if args[0] == "servers" {
					fmt.Printf("%s\n", serverType)
				}
			}

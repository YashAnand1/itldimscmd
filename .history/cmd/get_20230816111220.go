...
- itldims get <Server IP> <Attribute> | Display specific Attribute values of a Server Type     | Working
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Create a map to store attribute values along with server IPs
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			if !strings.Contains(key, "data") && (strings.Contains(key, args[0]) || strings.Contains(value, args[0])) {
				...
				for atr, values := range attributeValues {
					fmt.Printf("%s:\n", atr)
					...
					// Store the attribute value and its corresponding server IP
					attributeValues[serverAtr] = append(attributeValues[serverAtr], serverIP+":"+value)
				}
			}

			// Print the stored attribute values along with server IPs
			// for atr, values := range attributeValues {
			// 	fmt.Printf("%s:\n", atr)
			// 	for _, val := range values {
			// 		fmt.Printf("%s\n", val)

...
- itldims get <Server Type> <Attribute> | Display specific Attribute values of a Server Type | Working
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		//add the combo and non combo into the big if statement, combine both if

		// For 2 ARGUYMENTS - COMBO WITH KEY-VAL & KEY-KEY
		if len(args) == 2 && (args[0] == "servers" || args[0] == "types" || args[0] == "attributes") {
			for key, value := range parseKeyValuePairs(data) {
				if len(args) > 1 && !strings.Contains(key, "data") || strings.Contains(key, "{") || strings.Contains(key, "}") ||
					strings.Contains(value, "{") || strings.Contains(value, "}") {
					continue
				}
				// NO COMBINATION B/W KEY & VALUES OR KEY-KEY

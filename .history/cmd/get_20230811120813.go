...
- itldims get <Server IP> <Server Type> | Display Attribute values of a specific Server
	...
	Run: func(cmd *cobra.Command, args []string) { //This function is to be executed when 'get' subcommand is run
		...
		if len(args) == 1 && (args[0] == "servers" || args[0] == "server-types") { //If a single argument made is called servers, this will run
			servers := make(map[string]string) // Maps remove any duplicacy in the data. U
			...
			for key := range parseKeyValuePairs(data) {
				...
				servers[serverIP] = serverType
			}

			for serverIP, serverType := range servers {
				if args[0] == "servers" || args[0] == "server-types" {
					fmt.Printf("%s\n%s", serverIP, serverType)
				}
			}
			return
		}

...
var servers = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Printf("Failed to fetch data from the API: %v\n", err)
			return
		}

		serverAttributes := make(map[string]map[string]string)
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			serverIP := splitKey[3]
			serverAttr := splitKey[4]

			if _, exists := serverAttributes[serverIP]; !exists {
				serverAttributes[serverIP] = make(map[string]string)
			}

			serverAttributes[serverIP][serverAttr] = value
		}

		for serverIP, attrs := range serverAttributes {

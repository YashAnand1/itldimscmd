...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			if len(args) > 1 && !strings.Contains(key, args[1]) && !strings.Contains(value, args[1]) && strings.Contains(args[0], ".") && strings.Contains(key, args[0]) || strings.Contains(value, args[0]) {

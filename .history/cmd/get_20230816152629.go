	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			if len(args) == 1 || strings.Contains(args[0], ".") {
				// If there is only one argument, or it contains ".", display server attribute and value
				fmt.Printf("%s:\n%s\n", serverAtr, value)
			} else if (len(args) == 1 && !strings.Contains(args[0], ".")) && (strings.Contains(key, args[0]) || strings.Contains(value, args[0])) {
				// If there is only one argument and it doesn't contain ".", but key or value matches

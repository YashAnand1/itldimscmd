...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			if strings.Contains(args[0], ".") && strings.Contains(key, args[0]) || strings.Contains(value, args[1]) {
				if len(args) > 1 && !strings.Contains(key, args[1]) && !strings.Contains(value, args[0]) {
					continue
				}
				fmt.Printf("%s:\n%s\n\n", serverAtr, value)
			} else {

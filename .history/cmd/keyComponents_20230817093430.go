...
var servers = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) { // fetch data from API, parse key-value pairs, and display server IPs.
		...
		for key, value := range parseKeyValuePairs(data) { // the parsed data is looped and using "/" delimter, key divided into parts
			splitKey := strings.Split(key, "/")
			serverIP := splitKey[3]
			// the serverIP is the third index of the key
			IPs[serverIP] = value
			// the third index is then saved to the IPs
		}

		for serverIP, value := range IPs {
			fmt.Printf("%s\n", serverIP, value)
			fmt.Printf("----------------------------\n")
		}
	},

...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			valueRequested := len(args) == 2

			if strings.Contains(args[0], ".") && serverIP == args[0] && (strings.Contains(key, args[1]) || strings.Contains(value, args[1])) {
				if valueRequested && !strings.Contains(key, args[1]) && !strings.Contains(value, args[1]) {
					continue
				}
				fmt.Printf("%s:\n%s\n\n", serverAtr, value)
			} else if valueRequested && (strings.Contains(value, args[0]) || strings.Contains(value, args[1])) {
